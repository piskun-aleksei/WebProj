...
<html>
<head>
    <title><fmt:message key="Label.addBonusHead" bundle="${rb}"/></title>
...
<article class="container">
    ...
    <form name="DeleteUserForm" action="controller" method="POST">
        <select class="select-center" name="userId" required>
            <c:forEach var="elem" items="${nonAdmins}" varStatus="status">
                <option value="${elem.id}">${elem.login}</option>
            </c:forEach>
        </select>
        <input type="text" name="bonus" placeholder="<fmt:message key="Label.userBonus" bundle="${rb}"/>" class="input-field" pattern="[1-9]{1,1}[0-9]{0,2}" required/>
        <input type="hidden" name="command" value="addUserBonus" />
        <input type="hidden" name="page" value="/jsp/profile.jsp" />
        <input type="submit" name="backButton" class="input-button" value=<fmt:message key="Button.addUserBonus" bundle="${rb}"/> />
    </form>
